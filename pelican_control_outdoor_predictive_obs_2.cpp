...
double xrNow, dxrNow, xr5, dxr5;
double yrNow, dyrNow, yr5, dyr5;
double zrNow, dzrNow, zr5, dzr5;
...
Module0::initializeDataSaving() {
	...
	if (fs2.is_open()){
		...
		fs2 << "x_pred" << "\t" << "y_pred" << "\t" << "z_pred" << "\t";
		fs2 << "dx_pred" << "\t" << "dy_pred" << "\t" << "dz_pred" << "\t";
		fs2 << "ux" << "\t" << "uy" << "\t" << "uz" << "\t";
...
{
	printParams();
	printf("\nSporadic controllo N %d  ", ExpertId);
