...
double dx_past_pred[RITARDO];//predizioni passate velocit�
...
double y_error_pred;
double z_error_pred;
double dx_error_pred; //errori predizione velocit�
...
double cumuldx_pred = 0; //integrale errore predizione velocit�
...
double obstacleAdder(double x_coordinate, double y_coordinate, double livello)
{
	...
	for (it = Obstacles.begin(); it != Obstacles.end(); ++it)
	{
		...
		//il nuovo livello � dato da il livello influenzato precedentemente e la nuova gaussiana "SUPERCAZZOLA"
...
Module0::DoYourDuty(int wc)
{
	...
	*/
	...
	//Previsione velocit� attuale (con angolo attuale)	
	/*
	dxr += ax*mtime;
	dyr += ay*mtime;
	*/
